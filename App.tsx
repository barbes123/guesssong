	...
	const App: React.FC = () => {
	  ...
	  const initializeRound = (roundId: number) => {
	    ...
	    const roundData = ROUND_DATA[roundId] || [];
  ...
  return (
    <div className="min-h-screen bg-slate-950 p-8 pt-24">
	  ...
	  const RoundView = () => {
	    ...
	    if (isR5) {
	      const roundData = ROUND_DATA[roundId] || []; if (!roundData || roundData.length === 0) return null;
	      const currentPlayer = gameState.players[gameState.currentPlayerIndex];
...
<div className="w-[450px] flex flex-col gap-10 relative">
  ...
  {modal?.isOpen && modal.position === 'inline' && (
    ...
    />
	    ...
	    if (isR6) {
	      const turnIdx = progress.currentTurnIndex || 0; const song = ROUND_DATA[roundId][0]?.songs[turnIdx]; const isSongUsed = progress.usedNotes?.has(`r6_final-${turnIdx}`); const currentPlayer = gameState.players[gameState.currentPlayerIndex];
	      return (
		<div className="min-h-screen bg-slate-950 p-8 pt-24">
		  <div className="max-w-[1600px] mx-auto flex gap-12">
		    <div className="flex-1 flex flex-col gap-12">
    ...
    <div className="min-h-screen bg-slate-950 flex items-center justify-center p-8">
	  ...
	  const StartView = () => (
	    <div className="min-h-screen bg-slate-950 p-6 pt-20">
	      <div className="max-w-[1400px] mx-auto text-center"><h1 className="text-7xl font-black text-white mb-6 tracking-tighter uppercase">{t.title}</h1><p className="text-xl text-slate-400 mb-10 font-bold uppercase tracking-[0.4em]">{t.chooseRound}</p>
		...
		<div className="grid grid-cols-2 md:grid-cols-3 gap-8">{[1, 2, 3, 4, 5, 6].map((num, idx) => (
		    <button key={num} onClick={() => navigateTo('round', num)} className="group bg-slate-900 border-2 border-slate-800 p-10 rounded-[3rem] hover:border-indigo-500 transition-all hover:-translate-y-3 shadow-2xl"><MusicIcon size={60} className="text-slate-600 mx-auto mb-6" /><div className="text-5xl font-black text-white mb-2">{num === 6 ? t.categories.superGame : `${t.round} ${num}`}</div><div className="text-base font-black text-slate-500 uppercase mt-3">{num === 6 ? 'FINAL' : num === 5 ? 'SPRINT' : 'CHALLENGE'}</div></button>))}
