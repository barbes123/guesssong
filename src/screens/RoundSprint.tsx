...
import { getRoundData, SFX } from '../../data/index_data'; // Adjust path
...
const RoundSprint: React.FC<RoundSprintProps> = ({
  ...
  // Map Round 4 to old Round 5 data if needed
  const dataRoundId = roundId === 4 ? 5 : roundId; 
              ...
              <div className="flex gap-4">
                <button 
                  ...
                  className={`p-6 rounded-2xl transition-all ${
                    isPlaying ? 'opacity-20 cursor-not-allowed bg-slate-800' : 'bg-slate-800 text-slate-300 hover:bg-slate-700 hover:scale-105 active:scale-95 shadow-lg'
                  }`}
                  disabled={isPlaying}
                >
                  <ChevronLeft size={40} />
                      ...
                      {Array.from({length: 7}, (_, i) => startIdx + i).map(songIdx => {
                        ...
                        return (
                          <button 
                            ...
                            className={`h-24 rounded-2xl border-3 transition-all flex flex-col items-center justify-center ${
